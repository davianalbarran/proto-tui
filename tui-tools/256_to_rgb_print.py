...
for code in range(0, 16):
    ...
    # keep numeric since these are manually named in Kotlin
    print(f"{code:3d}(0x{r:02X}, 0x{g:02X}, 0x{b:02X}),")
...
for red in range(0, 6):
    for green in range(0, 6):
        for blue in range(0, 6):
            ...
            name = f"RGB_{r:02X}_{g:02X}_{b:02X}"
            print(f"{name}(0x{r:02X}, 0x{g:02X}, 0x{b:02X}),")
...
for gray in range(0, 24):
    ...
    name = f"GRAY_{level:02X}"
    print(f"{name}(0x{level:02X}, 0x{level:02X}, 0x{level:02X}),")